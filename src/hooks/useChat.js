...
export const useChat = ({ userId }) => {
  ...
  useEffect(() => {
    ...
    console.log('[useChat] Initializing socket connection...', { userId, SOCKET_URL });
    socketRef.current = io(SOCKET_URL, { transports: ['websocket'] });

    socketRef.current.on('connect', () => {
      console.log('[useChat] Socket connected successfully', { socketId: socketRef.current.id });
      ...
      if (activeChatIdRef.current) {
        console.log('[useChat] Re-fetching messages for active chat on reconnect', { 
      ...
      if (pendingChatFetchRef.current) {
        console.log('[useChat] Retrying pending chat fetch on reconnect', pendingChatFetchRef.current);
        const { chatId, page: fetchPage, limit } = pendingChatFetchRef.current;
        socketRef.current.emit('fetch_chat', { chatId, page: fetchPage, limit });
        socketRef.current.emit('chat_history', { chatId, page: fetchPage, limit });
    ...
    socketRef.current.on('all_chats', (chats) => {
      console.log('[useChat] Received all_chats event', { chatsCount: chats?.length || 0, chats });
      setChats(chats);
    });
    ...
    socketRef.current.on('chat_history', (data) => {
      ...
      if (isLoadMore) {
        ...
        setMessages(prev => {
          ...
          const newMessages = messagesArray.filter(m => m._id && !existingIds.has(m._id));
        ...
        console.log('[useChat] Set initial messages', { messagesCount: messagesArray.length });
        // Check if there are more messages to load
        if (messagesArray.length < 20) {
          setHasMore(false);
        } else {
    ...
    socketRef.current.on('receive_message', msg => {
      console.log('[useChat] Received new message', { message: msg, activeChatId });
      ...
      setChats(prev => {
        return prev.map(chat => chat?._id === msg.chat?._id ? { ...chat, lastMessage: {content:msg?.content,createdAt:msg?.createdAt,
          sender:msg?.sender
        } ,
          newMessage: true
         } : chat);
    ...
    socketRef.current.on('message_sent', msg => {
      console.log('[useChat] Message sent confirmation received', { message: msg });
      setMessages(prev => {
        const updated = [msg, ...prev];
    ...
    socketRef.current.on('message_seen', msg => {
      console.log('[useChat] Message seen confirmation received', { message: msg });
      setMessages(prev => {
        const updated = prev.map(m => (m._id === msg._id ? { ...m, status: 'seen' } : m));
    ...
    socketRef.current.on('user_typing', ({ senderId }) => {
      console.log('[useChat] User typing event received', { senderId, activeChatId });
      setTypingUser(senderId);
    });

    socketRef.current.on('user_stop_typing', () => {
      console.log('[useChat] User stop typing event received', { activeChatId });
      setTypingUser(null);
    });
    ...
    socketRef.current.on('chat_created', (chat) => {
      ...
      if (!activeChatId && chat) {
        ...
        console.log('[useChat] Updated activeChatId to new chat', { chatId: chat._id });
      }
    ...
    socketRef.current.on('chat_found_or_created', (data) => {
      ...
      if (chat && chat._id) {
        ...
        console.log('[useChat] Updated activeChatId from fetch_or_create', { chatId: chat._id });
        // Fetch messages for this chat
        if (chat.messages) {
          console.log('[useChat] Messages included in chat response', { messagesCount: chat.messages?.length || 0 });
          ...
          console.log('[useChat] Fetching chat history after chat found/created', fetchParams);
          // Only emit if socket is connected
          if (socketRef.current && isConnectedRef.current) {
            // Emit fetch_chat first, then chat_history
            socketRef.current.emit('fetch_chat', fetchParams);
            console.log('[useChat] Emitting chat_history event after fetch_chat', fetchParams);
            socketRef.current.emit('chat_history', fetchParams);
          } else {
            console.log('[useChat] Socket not connected, storing pending fetch', fetchParams);
  ...
  const fetchOrCreateChat = (receiverId) => {
    console.log('[useChat] fetchOrCreateChat called', { senderId: userId, receiverId });
    setMessages([]);
    setPage(1);
    setHasMore(true);
    setLoadingMessages(true);
    console.log('[useChat] Reset chat state for new chat', { messagesCount: 0, page: 1, hasMore: true });

    const chatData = {
      senderId: userId,
      receiverId: receiverId,
    };
    console.log('[useChat] Emitting fetch_or_create_chat_between_users event', chatData);
  ...
  const openChat = (chatId, receiverId) => {
    ...
    if (!chatId && receiverId) {
      console.log('[useChat] No chatId provided, fetching or creating chat between users');
    ...
    setHasMore(true);
    setLoadingMessages(true);
    console.log('[useChat] Reset chat state', { chatId, messagesCount: 0, page: 1, hasMore: true });
  ...
  const loadMoreMessages = () => {
    ...
    console.log('[useChat] Emitting chat_history event for loadMore', fetchParams);
    socketRef.current.emit('chat_history', fetchParams);
  ...
  const sendMessage = (receiverId, content, isNewChat = false) => {
    ...
    if (isNewChat && !activeChatId) {
      console.log('[useChat] Sending first message to create new chat', messageData);
    }
  ...
  const deleteMessage = messageId => {
    console.log('[useChat] deleteMessage called', { messageId, senderId: userId, activeChatId });
  ...
  const startTyping = receiverId => {
    console.log('[useChat] startTyping called', { senderId: userId, receiverId, activeChatId });
  ...
  const stopTyping = receiverId => {
    console.log('[useChat] stopTyping called', { senderId: userId, receiverId, activeChatId });
