...
const TaskUser = ({ navigation, route }) => {
    ...
    // User Hook
    const { users, loading, refreshing, loadMore, onRefresh, loadingMore, fetchUsers } = useUsers();
    ...
    const handleCreateTask = async () => {
        ...
        try {
            ...
            if (previousData.pictures && previousData.pictures.length > 0) {
                const uploadPromises = previousData.pictures.map(file => uploadFile(file));
    ...
    const renderUser = ({ item }) => (
        ...
        >
            <Image
                source={{ uri: item.profileImage || item.avatar || "https://randomuser.me/api/portraits/men/1.jpg" }}
                style={styles.avatar}
            />
